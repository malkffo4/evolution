...
#define PATTERN_MINE_MAX_GROUPS   16
...
int vm_op_mine_causal_pattern(VMContext *ctx, const Instruction *ins) {
    ...
    if (ctx->reg[r_subject].type != REG_NODE && ctx->reg[r_subject].type != REG_INT)
    ...
    int32_t min_count = (ctx->reg[r_min_count].type == REG_INT)
        ? (int32_t)ctx->reg[r_min_count].i : 3;

    // idx_causal_rev: cause_id -> child_id (DUPSORT). Seek O(log N) + скан
    // ограниченного числа дубликатов — НЕ полный скан таблицы.
    MDB_cursor *cur;
    ...
    Group groups[PATTERN_MINE_MAX_GROUPS];
        ...
        if (val.mv_size == sizeof(node_id_t)) {
            ...
            if (mdb_get(ctx->memory.txn, ctx->hyper_mem->dbi_atoms, &k2, &d2) == MDB_SUCCESS &&
                ...
                if (!found_group && group_count < PATTERN_MINE_MAX_GROUPS) {
                    ...
                    groups[group_count].sample = child.id;
                    group_count++;
                }
            }
        }
