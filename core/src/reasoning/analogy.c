...
#include "storage/vector_store.h"  // если нужно load_embedding
#include "math/vector_math.h"  // semantic_distance_u64

// Функция для поиска изоморфных подграфов
// int find_isomorphic_subgraphs(Edge *query_edges, int query_count, Edge **result_edges, int *result_count) {
...
//     // TODO: Реализовать поиск изоморфных подграфов для аналогий
//     return 0;
// }
...
static float node_similarity(MDB_txn *txn, node_id_t a, node_id_t b) {
    ...
    // Если есть simhash, используем его (чем меньше расстояние, тем выше сходство)
    uint64_t ha = na.simhash;
    uint64_t hb = nb.simhash;
    if (ha != 0 && hb != 0) {
        int dist = semantic_distance_u64(ha, hb);
        // 64-битный хеш, макс расстояние 64 -> нормируем
        return 1.0f - (float)dist / 64.0f;
    }
    // fallback: сравнение name_hash
    if (na.name_hash == nb.name_hash) return 1.0f;
    return 0.0f;
}

static inline float relation_similarity(
    relation_id_t a,
    relation_id_t b)
{
    return (a == b);
...
//
// greedy не оптимален.
//Позже стоит заменить на
// Hungarian Assignment Algorithm
// или
// Auction algorithm
// Но пока greedy — нормально.
static int match_edges(
    MDB_txn *txn,
    EdgeList *query,
    EdgeList *candidate,
    EdgeMatch **matches,
    ...
    *count = 0;
    ...
    for (int i = 0; i < query->count; i++)
    {
        ...
        if (best_j >= 0)
        {
            ...
            result[matched].query = query->edges[i].triple;
            result[matched].candidate = candidate->edges[best_j].triple;
    ...
    {
        EdgeMatch *tmp =
            realloc(result, matched * sizeof(EdgeMatch));
...
// Сравнение двух рёбер: учитываем relation и похожесть target
static float edge_similarity(MDB_txn *txn, const Edge *e1, const Edge *e2) {
    ...
    float rel_sim = relation_similarity(e1->triple.relation, e2->triple.relation);
...
int find_analogous_patterns(
    MDB_txn *txn,
    node_id_t query_node,
    AnalogyCandidate **candidates,
    int *candidate_count)
{
    int rc = MDB_SUCCESS;
...
#define MAX_CANDIDATES 128
    ...
    while ((rc = mdb_cursor_get(cursor, &key, &data, MDB_NEXT)) == MDB_SUCCESS)
    {
        ...
        if (rc != MDB_SUCCESS)
            goto cleanup_candidate;

        rc = get_edges_from_node(txn, cand_id, &cand_out);
        if (rc != MDB_SUCCESS)
            goto cleanup_candidate;

        AnalogyEvaluation eval =
            analogy_fast_score(
                txn,
                query_node,
                cand_id,
    ...
    const ReasoningStrategy *strategy)
{
    ...
    EdgeMatch *in_matches = NULL;
    EdgeMatch *out_matches = NULL;
    ...
    for (int i = 0; i < in_count; i++)
        in_sum += in_matches[i].similarity;

    float out_sum = 0.f;

    for (int i = 0; i < out_count; i++)
        out_sum += out_matches[i].similarity;

    eval.score.incoming =
        in_count ? in_sum / in_count : 0.f;

    eval.score.outgoing =
        out_count ? out_sum / out_count : 0.f;

    eval.score.center =
        node_similarity(txn, query_node, candidate);

    float in_cov =
        coverage_score(in_count, in_edges->count, cand_in->count);

    float out_cov =
        coverage_score(out_count, out_edges->count, cand_out->count);
    ...
    eval.score.neighborhood =
        0.5f * eval.score.incoming +
        0.5f * eval.score.outgoing;
    // TODO
    // center node degree
    // incoming count
    // outgoing count
    ...
    for (int i = 0; i < in_count; i++)
    {
        rel_sum += relation_similarity(
            in_matches[i].query.relation,
    ...
    if (in_count) {
        ...
        for (int i = 1; i < in_count; i++) {
            if (in_matches[i].similarity >
                eval.best_incoming.similarity)
            {
    ...
    if (out_count) {
        ...
        for (int i = 1; i < out_count; i++) {
            if (out_matches[i].similarity >
                eval.best_outgoing.similarity)
            {
