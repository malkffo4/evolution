...
int init_lmdb(const char *db_path) {
    ...
    rc = mdb_dbi_open(txn, "hyper_idx_causal_rev", MDB_CREATE | MDB_DUPSORT, &db.graph.hyper.idx_causal_rev);
    if (rc != MDB_SUCCESS) goto fail;
    ...
    rc = mdb_dbi_open(txn, "hyper_idx_pending", MDB_CREATE | MDB_DUPSORT, &db.graph.hyper.idx_pending);
    if (rc != MDB_SUCCESS) goto fail;
