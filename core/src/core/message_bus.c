...
void bus_stop(void) {
    // Безопасно переводим шины в статус остановки и будим потоки.
    // Примитивы синхронизации (mutex/cond) не уничтожаются здесь, так как другие потоки могут
    // находиться в процессе завершения. Они будут очищены позже в ipc_shutdown().
    pthread_mutex_lock(&tx_bus.mutex);
    tx_bus.running = false;
    pthread_cond_broadcast(&tx_bus.cond);
    pthread_mutex_unlock(&tx_bus.mutex);

    pthread_mutex_lock(&rx_bus.mutex);
    rx_bus.running = false;
    pthread_cond_broadcast(&rx_bus.cond);
    pthread_mutex_unlock(&rx_bus.mutex);
}
