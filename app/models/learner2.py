...
class CognitiveLearner:
    ...
    def extract_semantic_structure(self, text_chunk: str) -> dict:
        ...
        system_instruction = (
            ...
            "  \"nodes\": [\n"
            "    {\n"
            "      \"id\": \"уникальное_имя_сущности_в_нижнем_регистре\",\n"
            ...
            "}\n"
            "Блок properties обязателен. Оцени опасность (danger) и полезность (utility) от 0.0 до 1.0. Используйте только факты из текста. Все id делайте строго на латинице (например, 'c_language', 'buffer_overflow')."
        )
    ...
    def learn_text(self, text: str):
        ...
        for idx, chunk in enumerate(chunks):
            ...
            # На стороне C-ядра функция 'perceive_and_activate' примет структуру и запишет её в LMDB
            try:
                # Отправляем пакет напрямую в обработчик 'learn' (а не в chat)
                resp = self.ipc.command("learn", json.dumps({
                    "nodes": nodes,
                    "edges": edges
                }))
                
                print(f"[Learner] Ядро успешно приняло и упаковало структуру. Ответ: {resp}")
            except Exception as e:
